...
BOOL ProcessButtonEvent()
{
	...
	if(i < nNumCommand)
	{
		if(nNumCommand == MAX_UTTERANCE * 2 && i == 0)
		{
			for(int j = i, k = i + MAX_UTTERANCE; k < nNumCommand; j++, k++)
			{
				psPtr[j] = psPtr[k];
			}
		}
		else if(nNumCommand == MAX_UTTERANCE * 3 && i == 0)
		{
			for(int j = i, k = i + MAX_UTTERANCE; k < nNumCommand; j++, k++)
			{
				psPtr[j] = psPtr[k];
		...
		else if(nNumCommand == MAX_UTTERANCE * 3 && i == MAX_UTTERANCE)
		{
			for(int j = i, k = i + MAX_UTTERANCE; k < nNumCommand; j++, k++)
			{
				psPtr[j] = psPtr[k];
		...
		*((INT *)psPtr) = nNumCommand - MAX_UTTERANCE;
...
//	am_hal_flash_page_erase(AM_HAL_FLASH_PROGRAM_KEY, 1, FLASH_PAGE_INDEX);
	...
	if(hDSpotter)
	{
		...
		nErr = DSpotter_SetConfiReward(hDSpotter, CONFIDENCE_REWARD);
		if(nErr != DSPOTTER_SUCCESS){
				am_app_utils_stdio_printf(2, "DSpotter_SetConfiReward Fail!!(%d)\r\n", nErr);
				return NULL;
		}
		...
		if(g_bTrainSDModel)
		{
				...
				else if(nErr == DSPOTTER_SUCCESS)
				{
						...
						if(++nUtterance >= MAX_UTTERANCE)
						{
								...
								{
										psPtr[nNumCommand + i] = g_sTargetID;
								}
								psPtr = g_lpsMapID + 160;
								*((INT *)psPtr) = nNumCommand + MAX_UTTERANCE;
								...
								nErr = DSpotter_SetConfiReward(g_hDSpotter, CONFIDENCE_REWARD);
								if(nErr != DSPOTTER_SUCCESS)
								{
										AM_APP_LOG_WARNING("DSpotter_SetConfiReward Fail!!(%d)\n", nErr);
										bErrorOccurred = TRUE;
										return;
								}
